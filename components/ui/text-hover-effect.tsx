...
export const TextHoverEffect = ({
  ...
  // start cursor as null so the initial mask remains centered
  const [cursor, setCursor] = useState<{ x: number; y: number } | null>(null);
  const [hovered, setHovered] = useState(false);
  const [maskPosition, setMaskPosition] = useState({ cx: "50%", cy: "50%" });

  useEffect(() => {
    // only update mask when we actually have cursor coordinates
    if (!cursor || !svgRef.current) return;

    const svgRect = svgRef.current.getBoundingClientRect();
    const cxPercentage = Math.max(
      0,
      Math.min(100, ((cursor.x - svgRect.left) / svgRect.width) * 100),
    );
    const cyPercentage = Math.max(
      0,
    ...
    >
      <defs>
        <linearGradient
          ...
          r="25%"
        >
          {/* Always render subtle gradient stops so the watermark is visible by default */}
          <stop offset="0%" stopColor="#eab308" stopOpacity="0.55" />
          <stop offset="25%" stopColor="#ef4444" stopOpacity="0.45" />
